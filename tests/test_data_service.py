...
def test_sync_prices_handles_duplicates_and_replaces(mocker):
    ...
    - Simula prezzi esistenti nel DB.
    - Simula il download di nuovi prezzi (con sovrapposizioni).
    - Controlla che il DataFrame finale salvato sia corretto (senza duplicati, con i valori più recenti).
    ...
    # Verifica che il DataFrame salvato abbia 2 righe (il duplicato è stato gestito)
    assert len(saved_df) == 2
    
    # Verifica che il prezzo per la data duplicata ('2025-12-19') sia quello nuovo (105.0)
    price_on_duplicate_date = saved_df[saved_df['date'] == pd.to_datetime('2025-12-19')]['close_price'].iloc[0]
